...
import {LOGIN_MUTATION, SIGNUP_MUTATION} from '../queries/posts'
...
export  default function Login ({history}){
    ...
    const setModel = useCallback((key) => ({target}) => {
        setData({
            // It will iterate through the data values
            ...data,
            [key]: target.value
        })
    }, [data])
    ...
    const submit = (
        <Button className  ="btn-small" type="flashy" htmlType="submit" onClick={onSubmit}>
            { activeKey === '1' 
                ? 'Login'
                : activeKey === '2'
                ? 'Sign up'
